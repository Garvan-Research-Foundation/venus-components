...
function findDeclareSourceNode(ast, name) {
    ...
    var init = lodash.get(found, 'init');
    if (!init) {
        return undefined;
    }
...
function appendExport(id, name, moduleName, ast, magicString, appendList) {
    ...
    if (!findExportedNode(ast, name)) {
        var hashedName = rollupPluginutils.makeLegalIdentifier(name + "_" + hashString(id, name));
...
function namedExport(options) {
    ...
    return {
        ...
        transform: function (code, id) {
            ...
            var moduleName = lodash.get(commonjsExported, 'local.name');
            var nodeValue = findDeclareSourceNode(ast, moduleName);
            var properties = lodash.get(nodeValue, 'properties');
            if (properties) {
            ...
            var members = lodash.get(nodeValue, 'members');
            if (members) {
